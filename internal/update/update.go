...
const (
	releaseURL      = "https://api.github.com/repos/aiomayo/hdf/releases/latest"
...
func shouldSkip(currentVersion string) bool {
	if os.Getenv("HDF_NO_UPDATE_CHECK") == "1" {
		return true
	}
...
func fetchLatestVersion(cached *cache) (version, etag string, err error) {
	...
	req, err := http.NewRequest(http.MethodGet, releaseURL, http.NoBody)
	if err != nil {
		return "", "", err
	}

	req.Header.Set("User-Agent", "hdf")
